  ...
  return (
    <div className="min-h-screen bg-bone">
      ...
      <div className="mx-auto max-w-6xl px-6 py-12">
        ...
        <p className="font-body text-sm text-soil/55 mb-8">
          Work through the Open Season steps first, then the Closed Season steps.
        </p>
        ...
        <div className="flex gap-1 border-b border-soil/10 mb-8">
          {tabs.map((tab) => (
            <button
              ...
              className={`px-5 py-2.5 font-display font-black text-sm transition rounded-t-lg ${
                activeTab === tab.key
                  ? "bg-soil text-redoxide"
                  : "text-soil/45 hover:text-soil"
              }`}
            >
              {tab.label}
            </button>
        ...
        {loading ? (
          <p className="font-mono text-sm text-soil/50">Loading…</p>
        ) : (
              ...
              {selected && (
                <div className="mt-4">
                  ...
                  {selected.description && (
                    <p className="mt-1 font-body text-sm leading-relaxed text-soil/60">{selected.description}</p>
                  )}
                </div>
              )}
            </div>
                ...
                <li key={video.id}>
                  <button
                    ...
                    className={`flex w-full items-start gap-3 rounded-xl px-3 py-3 text-left transition ${
                      selected?.id === video.id ? "bg-soil" : "hover:bg-soil/5"
                    }`}
                  >
                    ...
                    {selected?.id !== video.id && (
                      <PlayCircle size={14} className="ml-auto mt-0.5 shrink-0 text-soil/20" />
                    )}
                  </button>
                </li>
              ))}
