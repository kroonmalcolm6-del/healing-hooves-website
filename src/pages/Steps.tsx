  ...
  return (
    <div className="min-h-screen bg-bone">
      ...
      <div className="mx-auto max-w-6xl px-6 py-12">
        ...
        {error && (
          <div className="rounded-xl bg-red-50 border border-red-200 p-4 mb-6 font-body text-sm text-red-700">
            Error: {error}
          </div>
        )}
        ...
        {!loading && !error && videos.length === 0 && (
          <p className="font-mono text-sm text-soil/50">No videos found in database.</p>
        )}
              ...
              {tabs.map((tab) => (
                <button
                  ...
                  className={`px-5 py-2.5 font-display font-black text-sm transition rounded-t-lg ${
                    activeTab === tab.key ? "bg-soil text-redoxide" : "text-soil/45 hover:text-soil"
                  }`}
                >
                  {tab.label}
                </button>
